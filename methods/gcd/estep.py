...
def run_estep(labelled_logits: torch.Tensor, labelled_labels: torch.Tensor,
              ...
              unlab_labels: torch.Tensor = None,
              prev_assign_unlab: Optional[torch.Tensor] = None,
              frozen_lda: Optional["LDAGaussian"] = None) -> EStepResult:
    # unlab_labels: optional GT labels for the unlabelled split. DIAGNOSTIC ONLY (logging);
    # never used to make a training decision here, so no label leakage.
